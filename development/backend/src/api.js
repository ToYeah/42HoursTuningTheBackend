...
const postRecords = async (req, res) => {
  ...
  if (rows.length !== 1) {
    console.log('申請者のプライマリ組織の解決に失敗しました。');
    res.status(400).send();
    return;
  }
...
const normalizeRecord = (records) => {
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    records[i].isUnConfirmed = true;
    if (records[i].accessTime && Date.parse(records[i].updated_at) <= Date.parse(records[i].accessTime)) {
      records[i].isUnConfirmed = false;
    }
    delete records[i].accessTime;


    if (!records[i].createdByName) {
      records[i].createdByName = null;
    }

    if (!records[i].applicationGroupName) {
      records[i].applicationGroupName = null;
    }

    if (!records[i].thumbNailItemId) {
      records[i].thumbNailItemId = null;
    }

    if (!records[i].commentCount) {
      records[i].commentCount = 0;
    }
...
const tomeActive = async (req, res) => {
  ...
  FROM record \
  WHERE status = "open" \
  AND (category_id, application_group) in (select category_id, application_group  from category_group where group_id IN (select group_id from group_member where user_id = ?)) \
  LIMIT 1;`;
  ...
  const [recordResult] = await pool.query(searchRecordQs, [user.user_id, user.user_id, limit, offset]);

  res.send({ count: count, items: normalizeRecord(recordResult) });
};
...
const allActive = async (req, res) => {
  ...
  const recordCountQs = 'select count(*) from record where status = "open" limit 1';
